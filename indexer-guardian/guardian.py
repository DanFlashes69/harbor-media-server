...
SLOT_DEFINITIONS: dict[str, dict[str, Any]] = {
    ...
    "general_secondary": {
        "seedDefinition": "limetorrents",
        "priority": 20,
        "appProfileId": 1,
        "replacementPool": ["limetorrents", "torrentdownload", "torrentproject2", "0magnet", "magnetdownload", "rutor"],
        "testQuery": {"q": "matrix", "cat": "2000"},
    },
    "movies_primary": {
        "seedDefinition": "yts",
    ...
    "general_tertiary": {
        "seedDefinition": "thepiratebay",
        "priority": 35,
        "appProfileId": 1,
        "replacementPool": ["thepiratebay", "torrentproject2", "torrentdownload", "0magnet", "magnetdownload", "rutor"],
        "testQuery": {"q": "matrix", "cat": "2000"},
    },
}
...
def parse_warning_indexers(messages: list[dict[str, Any]]) -> set[str]:
    ...
    for item in messages:
        if item.get("source") != "IndexerLongTermStatusCheck":
            continue
        message = str(item.get("message") or "")
        marker = "Indexers unavailable due to failures for more than 6 hours:"
...
def note_replacement_history(store: StateStore, slot_name: str, definition_name: str, reason: str) -> None:
    ...
    store.runtime["lastReplacementAt"] = now_ts()
...
def replacement_due(
    ...
    if current_test_ok:
        ...
        return False, {"warningPresent": warning_present, "failureCount": 0}

    entry["count"] = int(entry.get("count", 0)) + 1
    entry["lastFailureTs"] = now_ts()
    due = entry["count"] >= CONFIG.replacement_failure_threshold or warning_present
    return due, {"warningPresent": warning_present, "failureCount": entry["count"]}


def replacement_cooldown_remaining(store: StateStore) -> int:
...
def execute_cycle(store: StateStore) -> dict[str, Any]:
    ...
    warning_names = set().union(*warning_names_by_app.values())
    ...
    slot_reports: dict[str, Any] = {}
    cooldown_remaining = replacement_cooldown_remaining(store)
    ...
    for slot_name, slot_info in slot_state.items():
        ...
        if current is None:
            candidate_definition = choose_candidate_definition(slot_name, active_definitions, store)
            report["current"] = {"missing": True, "expectedDefinition": slot_info.get("definitionName")}
            if candidate_definition and cooldown_remaining == 0:
        ...
        due, due_meta = replacement_due(store, slot_name, str(current.get("name") or ""), ok, warning_names)
        report["replacementGate"] = due_meta

        if ok or not due or cooldown_remaining > 0:
            slot_reports[slot_name] = report
            continue

        candidate_definition = choose_candidate_definition(slot_name, active_definitions - {current.get("definitionName")}, store)
        ...
        if not template:
            report["replacementAttempted"] = False
            report["replacementSkippedReason"] = "schema-missing"
            slot_reports[slot_name] = report
            continue
    ...
    snapshot = {
        ...
        "warnings": {key: sorted(value) for key, value in warning_names_by_app.items()},
        "warningHealPlan": warning_heal_plan,
        "staleAppIndexers": stale_indexer_cleanup,
        "applicationSyncs": application_syncs,
        "cooldownRemainingSeconds": replacement_cooldown_remaining(store),
        "actions": actions,
        "slots": slot_reports,
